...
SET FFU_FILE_NAME=%REPO_BUILD_ROOT%\solution\iotPlatform\Build\FFU\%BOARD_NAME%\%FFU_FILE_NAME%
...
REM
set BSPPKG_DIR=%REPO_BUILD_ROOT%\solution\iotPlatform\Build\FFU\bspcabs\%PLATFORM%\%Configuration%
set PKG_VER=1.0.0.0
if not exist %BSPPKG_DIR% ( mkdir %BSPPKG_DIR% )

if not exist %REPO_BUILD_ROOT%\..\..\b\%Configuration%\%PLATFORM% (dir /S /B %REPO_BUILD_ROOT%\solution\iotPlatform\Build\%PLATFORM%\%Configuration%\*.cab > filelist.txt)
...
if not exist %REPO_BUILD_ROOT%\..\..\b\%Configuration%\%PLATFORM% (dir /S /B %REPO_BUILD_ROOT%\solution\iotPlatform\Build\%PLATFORM%\%Configuration%\%BOARD_NAME%\*.cab >> filelist.txt)
